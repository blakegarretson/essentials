...
- [Obsidian](https://obsidian.md) is where I go when I *don't* need integrated multimedia in my notes. It's a Markdown-based framework that organizes collections of text files, so it lends itself to note-taking, but it could be used effectively to do anything from mind-mapping to novel writing. The files are plain text, so it works well with version control (e.g. git), and there are clients for all popular platforms. I use Google Drive to sync across platforms, but you could use any number of solutions, including paying for Obsidian Sync, which is an available upgrade.
